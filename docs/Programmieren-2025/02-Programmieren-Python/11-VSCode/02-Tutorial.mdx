...
- Um eine Datei auf die äusserste Ebene (also direkt in den Projektordner) zu verschieben, ziehen Sie die Datei in den leeren Bereich im Dateibaum (unterhalb aller Ordner und Dateien).
</Video>
...
Womöglich erscheint anschliessend ein Bildschirm zur Ersteinrichtung von VSCode. Diesen können Sie einfach mit __Mark Done__ schliessen, ohne die angebotenen Optionen anzuwählen:
...
Achten Sie deshalb genau darauf, welcher Tab gerade aktiv ist, wenn Sie etwas schreiben oder ändern wollen, damit Sie nicht versehentlich die falsche Datei bearbeiten. In der Abbildung oben sehen Sie z.B. die Datei `hallo.py`, weil der entsprechende Tab hervorgehoben ist.
:::
