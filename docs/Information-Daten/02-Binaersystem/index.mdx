...
Genau so verhält es sich auch mit den Signalen, die wir in der Informatik verwenden. Je mehr Werte ein Signal annehmen kann, desto schwieriger ist es, diese Werte fehlerfrei zu lesen. Die Werte $0$ und $1$ lassen sich in der physikalischen Welt sehr gut darstellen, wenn wir davon ausgehen, dass keine Werte dazwischen möglich sind. Beispielsweise könnte der Wert $0$ durch keinen Stromfluss und der Wert $1$ durch einen Stromfluss dargestellt werden. Solange es keinen Stromfluss gibt, können wir sicher sein, dass der Wert $0$ gemeint ist. Sobald Strom fliesst, können wir sicher sein, dass der Wert $1$ gemeint ist. Es gibt hier also keine Möglichkeit für Missverständnisse oder Fehler.
